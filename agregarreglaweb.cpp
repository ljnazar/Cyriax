...
void AgregarReglaWeb::on_pushButton_clicked()
{
    ...
    {
        char sys1[50];
        sprintf(sys1,"echo -n ' -d %s' >> ./regla_pre",cadena1);
        system(sys1);
    }
    ...
    {
        char sys2[50];
        sprintf(sys2,"echo -n ' -m string --string \"%s\" --algo kmp' >> ./regla_pre",cadena2);
        system(sys2);
    }
    ...
    if(ui->timeEdit->isEnabled()==true)
    {
        ...
        {
            char sys3[50];
            sprintf(sys3,"echo -n ' -m time --timestart %s' >> ./regla_pre", timeaux2);
            system(sys3);
        }
    }
    ...
    if(ui->timeEdit_2->isEnabled()==true)
    {
        ...
        {
            char sys4[50];
            sprintf(sys4,"echo -n ' --timestop %s' >> ./regla_pre",timeaux4);
            system(sys4);
        }
